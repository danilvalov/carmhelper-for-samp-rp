...
class Updater
{
  ...
  Updated() {
    ...
    MsgBox, % "CarmHelper успешно обновлён до версии " Config["About"]["Version"] " (" Config["About"]["LastUpdate"] ").`nЖелаем приятного пользования.`n`nПосле нажатия кнопки ""OK"" запустится CarmHelper."
  ...
  CheckUpdate() {
    ...
    if (Config["About"]["Version"] < Repository_Data["tag_name"]) {
      MsgBox, 4, , % "Доступно обновление CarmHelper.ahk:`nВерсия: " Repository_Data["tag_name"] ".`n`nЧто нового:`n`n" StrReplace(Repository_Data["body"], "\r\n", "`n") "`n`nВы желаете обновиться прямо сейчас?"
      IfMsgBox, Yes
      {
